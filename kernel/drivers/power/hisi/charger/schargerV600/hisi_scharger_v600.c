...
static void hi6526_dbg_work(struct work_struct *work)
{
	...
	if (di->param_dts.ignore_dbg_check == IGNORE_DBG_TRUE)
		return;
...
*  return value:  0-success or others-fail
**********************************************************/
static int hi6526_dump_register(char *reg_value, int size)
{
        u8 reg_val = 0;
        char buff[BUF_LEN] = { 0 };
        int i = 0;
        ...
        errno_t err = EOK;
        struct hi6526_device_info *di = g_hi6526_dev;
        #define HI6526_DUMP_REGISTER_ERR_SIZE 8
        errno_t err_array[HI6526_DUMP_REGISTER_ERR_SIZE] = {0};

        if (NULL == di) {
                SCHARGER_ERR("%s hi6526_device_info is NULL!\n", __func__);
                return -ENOMEM;
        }
        err = memset_s(reg_value, size, 0, size);
		if(err != EOK) {
        ...
        err_array[0] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, BUF_LEN, 26, "%-8.2d", ibus);
        err_array[1] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26,"%-8.2d", vbus);
        err_array[2] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, BUF_LEN, 26,"%-8.2d", ibat);
        err_array[3] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "%-8.2d", vusb);
        err_array[4] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "%-8.2d", vout);
        err_array[5] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "%-8.2d", vbat);
        err_array[6] = strncat_s(reg_value, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "%-8.2d", I_bias_all);
        err_array[7] = strncat_s(reg_value, size, buff, strlen(buff));

        for (i = 0; i < HI6526_DUMP_REGISTER_ERR_SIZE; i++) {
                if(err_array[i] != EOK) {
                    SCHARGER_ERR("[%s]strncat_s[%d] fail, err=%d\n",__func__, i, err_array[i]);
                }
        }
        ...
        for (i = 0; i < (PAGE0_NUM); i++) {
                ...
                err = strncat_s(reg_value, size, buff, strlen(buff));
                if(err != EOK) {
...
*  return value:  0-success or others-fail
**********************************************************/
static int hi6526_get_register_head(char *reg_head, int size)
{
        char buff[BUF_LEN] = { 0 };
        int i = 0;
        errno_t err = EOK;
        #define HI6526_GET_REGISTER_HEAD_ERR_SIZE 8
        errno_t err_array[HI6526_GET_REGISTER_HEAD_ERR_SIZE] = {0};

        err = memset_s(reg_head, size, 0, size);
		if(err != EOK) {
			SCHARGER_ERR("[%s]memset_s fail, err=%d\n",__func__, err);
		}
        snprintf_s(buff, BUF_LEN, 26, "mode    ");
        err_array[0] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, BUF_LEN, 26, "Ibus    ");
        err_array[1] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Vbus    ");
        err_array[2] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Ibat    ");
        err_array[3] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Vusb    ");
        err_array[4] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Vout    ");
        err_array[5] = strncat_s(reg_head, size, buff, strlen(buff));
        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Vbat    ");
        err_array[6] = strncat_s(reg_head, size, buff, strlen(buff));

        snprintf_s(buff, (unsigned long)BUF_LEN, 26, "Ibias   ");
        err_array[7] = strncat_s(reg_head, size, buff, strlen(buff));

        for (i = 0; i < HI6526_GET_REGISTER_HEAD_ERR_SIZE; i++) {
                if(err_array[i] != EOK) {
                    SCHARGER_ERR("[%s]strncat_s[%d] fail, err=%d\n",__func__, i, err_array[i]);
                }
        }
        ...
        for (i = 0; i < (PAGE0_NUM); i++) {
                ...
                err = strncat_s(reg_head, size, buff, strlen(buff));
                if(err != EOK) {
                    SCHARGER_ERR("[%s]strncat_s fail, err=%d\n",__func__, err);
                }
        }
...
static int hi6526_fcp_reg_write_block(int reg, int *val, int num)
{
	int ret;
	int i;
...
static void parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	ret = of_property_read_u32(np, "ignore_dbg_check", (u32 *)&(di->param_dts.ignore_dbg_check));
	if (ret) {
		di->param_dts.ignore_dbg_check = IGNORE_DBG_FALSE;
		SCHARGER_ERR("get ignore_dbg_check failed\n");
	}
	SCHARGER_INF("prase_dts ignore_dbg_check = %d\n", di->param_dts.ignore_dbg_check);
