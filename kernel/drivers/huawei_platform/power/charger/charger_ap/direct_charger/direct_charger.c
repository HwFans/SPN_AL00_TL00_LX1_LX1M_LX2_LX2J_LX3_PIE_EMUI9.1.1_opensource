...
#define MASK_MULTI_WDT_TRUE     1
#define MASK_MULTI_WDT_FALSE    0
...
void direct_charge_kick_wtd_work(struct work_struct *work)
{
	...
	if (di->mask_multi_kick_wdt != MASK_MULTI_WDT_TRUE)
		direct_charge_get_bat_current(&ibat);
	...
	struct direct_charge_device *di)
{
	...
	ret = of_property_read_u32(np, "mask_multi_kick_wdt", &(di->mask_multi_kick_wdt));
	if (ret) {
		di->mask_multi_kick_wdt = MASK_MULTI_WDT_FALSE;
		hwlog_err("mask_multi_kick_wdt dts read failed\n");
	}
	hwlog_info("mask_multi_kick_wdt=%d\n", di->mask_multi_kick_wdt);
